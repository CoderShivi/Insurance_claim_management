sap.ui.define([
    ...
    if (!oTable) {
        console.error("[Claims] claimsTable not found.");
        return;
    }
    ...
    if (oBinding) {
        oBinding.attachDataReceived(function (oEvent) {
            ...
            if (oError) {
                ...
                console.log("[Claims] Claims loaded:", oBinding.getCurrentContexts().length);
            }
        });
    }

    // NEW — react to ?query=... coming from the dashboard KPI cards
...
_onRouteMatched: function (oEvent) {
    ...
    if (oQuery.status === "Pending") {
        aFilters.push(new Filter({
            filters: [
                new Filter("status", FilterOperator.EQ, "PendingApproval"),
                new Filter("status", FilterOperator.EQ, "UnderReview")
            ],
            and: false
        }));
    } else if (oQuery.status === "Rejected" || oQuery.status === "Submitted") {
        // NEW — exact status match for the Rejected / Submitted KPI cards
        aFilters.push(new Filter("status", FilterOperator.EQ, oQuery.status));
    } else if (oQuery.fraudRisk === "High") {
        aFilters.push(new Filter("fraudRiskScore/riskLevel", FilterOperator.EQ, "High"));
    }
